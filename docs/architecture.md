...
- **A single Rust binary.** The RAG layer ships as one executable, so a new
  user downloads a binary instead of installing a language toolchain, and the
  heaviest dependency of the earlier prototype (a multi-gigabyte PyTorch
  install) is gone.
