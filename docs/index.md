...
- A local **hybrid RAG index** over your vaults: a single Rust binary over
  SQLite + sqlite-vec + FTS5, vector search fused with BM25, optional MMR
...
- [RAG layer](rag.md) - the `eos-rag` binary, its search API, and model choice.
...
!!! note "Status"
    v0.1. The vault template and the Rust RAG layer are in place; skills,
    hooks, beads integration, and this docs site are landing next.
